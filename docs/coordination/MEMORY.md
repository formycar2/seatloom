...
- [~] SG-A §A4-β (Mira real-API wire-up) **DISPATCHED** 2026-05-09 evening per Aegis directive. Packet: `docs/coordination/tasks/mira/MIRA-2026-05-09-a4b-sessions-real-api-wire-v1.md`. Scope: replace MOCK_TMUX_SESSIONS + console.warn stubs in SessionsWorkspace.tsx with real `api.listTmuxSessions()` + `api.attachTmuxSession()`; add `TmuxSessionInfo` to types-dto; add 2 API wrappers to api.ts. Three files only (SessionsWorkspace + api.ts + types-dto). R1 attach-only + R3 failure-isolation + v0.0.1 read-only + disableStdin preserved; browser-dev degradation path preserved. tsc=0 + pnpm build=0 required. Flux Layer A verify follows Mira delivery. (2026-05-09, Lyra / Mira)
...
---
*Last Updated: 2026-05-09 evening (SG-A UNCONDITIONAL PASS closed; A3 acceptance promoted to UNCONDITIONAL; A4-β dispatched to Mira; B1 packet drafted for Nimbus, parked on A4-β PASS gate; SG-B unlocked; Aegis rulings on hotfix-packet + supplement-joint-review timing recorded)*
