...
- [~] SG-A §A4-β (Mira real-API wire-up) **DELIVERED → FLUX LAYER A VERIFY DISPATCHED** 2026-05-09 evening. Mira delivery at `docs/coordination/tasks/mira/MIRA-2026-05-09-a4b-sessions-real-api-wire-delivery-v1.md` @ commit `ab672e5` (3 files / 56 ins / 42 del; tsc=0, pnpm build green). Lyra pre-flight audit clean: scope 3/3 (SessionsWorkspace.tsx + api.ts + types-dto.ts), `TmuxSessionInfo` wire shape verified snake_case against Rust struct at `pty/mod.rs:113-119` (no `rename_all`), `AttachTmuxRequest` envelope verified camelCase against `session_cmds.rs:84-85`, MOCK_TMUX_SESSIONS + `console.warn('A3 not yet available')` removed, read-only banner preserved at line 185-187, v0.0.1 pill preserved at line 123, disableStdin untouched (file not in diff), isTauri() degradation + cancellation-safe useEffect verified. One carry-forward flagged non-blocking: `closeTab` at lines 72-78 does not call `api.killSession` so backend FIFO/pipe-pane leaks until app exit (matches A4-α behavior, not R3 violation since A3 backend `kill_session` is already R3-safe). Flux Layer A verify packet dispatched at `docs/coordination/tasks/flux/FLUX-2026-05-09-a4b-sessions-real-api-wire-verify-v1.md` with 13 static-invariant matrix + 5-row R-rule matrix + §5 explicit non-blocker list. Lyra acceptance held pending Flux PASS (three-layer gate protocol). On Flux PASS → Lyra acceptance → "B1 GO" dispatch to Nimbus. (2026-05-09, Lyra / Mira / Flux pending)
- [~] SG-B §B1 (Nimbus tmux send-keys bidirectional write path) **DRAFT** parked 2026-05-09 evening per Aegis directive. Packet: `docs/coordination/tasks/nimbus/NIMBUS-2026-05-09-b1-pty-write-send-keys-v1.md`. Scope (5 files): `crates/seatloom-core/src/pty/mod.rs` (replace `PtySession::write` no-op at line 324 with `tmux load-buffer | paste-buffer` path, recommended over argv send-keys for arbitrary-byte fidelity including NUL/ANSI control), `src-tauri/src/commands/session_cmds.rs` (cmd_pty_write + cmd_pty_write_bytes wire to `session.write()` at lines 270/281), `crates/seatloom-core/examples/tmux_mirror_smoke.rs` (Steps 10–12: write round-trip, Ctrl-C signal delivery, R3 still-alive after kill), `ui/src/app-v2/panel/SessionTerminal.tsx` (drop `disableStdin: true` line 66, restore `cursorBlink: true` line 64, wire `term.onData` → `api.ptyWriteBytes` at TODO(B2) line 80), `ui/src/app-v2/panel/SessionsWorkspace.tsx` (banner + pill amber/v0.0.1 → blue/v0.0.2). R1/R3 invariants preserved (send-keys is observe-level control, not ownership). Status holds at `draft` until A4-β Flux Layer A PASS lands. Lyra dispatches "B1 GO" to Nimbus only after that gate clears. (2026-05-09, Lyra / Nimbus pending dispatch)
...
---
*Last Updated: 2026-05-09 late-evening (A4-β DELIVERED by Mira @ ab672e5, Lyra pre-flight audit clean, Flux Layer A verify packet dispatched; Lyra acceptance held pending Flux PASS per three-layer gate; B1 still parked; SG-A UNCONDITIONAL PASS closed; SG-B unlocked; Aegis rulings on hotfix-packet + supplement-joint-review timing recorded)*
