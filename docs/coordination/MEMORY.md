...
- [x] **008 ACCEPTED PROVISIONAL** at `526d25a` — Layer A PASS (Flux 15/15 static + 4/4 R-rules; migration applied with verbatim psql output: BEGIN + 4× ADD COLUMN IF NOT EXISTS + UPDATE 6/5/3/35 + 8× DROP DEFAULT/SET NOT NULL + 4× CREATE INDEX + COMMIT; idempotent re-run NOTICE skips clean; all 4 NULL-counts=0; all 4 GROUP BYs show only 'seatloom'; tsc=0 + build green; cargo 60/60). Acceptance doc at `docs/coordination/acceptance/2026-05-09-lyra-nimbus-schema-008-project-isolation-acceptance-v1.md`. Three §10 pre-impl corrections applied during implementation (event_object_refs columns ref_type/ref_id; ref_type=seat-only forward-compat; ADD COLUMN IF NOT EXISTS PG 9.6+ no DO block). 4 new project-mode commands (cmd_list_*_for_project × 4) + un-scoped commands preserved for AD-013 v2 Global mode = backend mirror of frontend invariants. PROVISIONAL pending Mr. Zhang Layer B (4-step Tauri runtime smoke per acceptance §3); promotes to UNCONDITIONAL on Layer B PASS. **B2 onward unblocked** (canonical_events.project_id now exists; AD-013 v2 backend enforcement defendable in WHERE clauses). (2026-05-11, Lyra / Nimbus / Flux)
- [~] **Sessions tab mount packet dispatched to Mira** — `docs/coordination/tasks/mira/MIRA-2026-05-11-v1-sessions-tab-mount-v1.md`. P0 because B1 Layer B is blocked: Mr. Zhang has tmux sessions in `tmux ls` but no path from V1 NavRail to reach SessionsWorkspace Attach UI. Same R4 gap Aegis flagged in SG-A as deferred. Scope: 2-3 files (App.tsx Tab union + import + render switch case 'sessions'; NavRail.tsx items array adds `{ id: 'sessions', icon: Terminal, label: '会话' }`; optional types.ts NavTab union). No backend, no IPC, no DTO changes. Verify: pnpm tauri dev shows "会话" tab between Playbook and (was last); click renders SessionsWorkspace; dropdown lists `*-seatloom` tmux sessions. Once landed, B1 Layer B 6-step smoke runnable. (2026-05-11, Lyra → Mira; Aegis flagged as blocker)
- [x] **§A.6 closed** at commit `32fbc72`: Nimbus applied B1-parallel-clearance note to arch §3 Recommendation block per Aegis-approved verbatim wording. All 7 consolidation amendments now landed (§A.1-§A.5 by Lyra, §A.6 by Nimbus, §A.7 by Lyra with Aegis-revised wording). Joint-review chapter closed. (2026-05-11, Nimbus)
...
---
*Last Updated: 2026-05-11 (B1 + 008 both PROVISIONAL ACCEPTED with Layer A PASS; Aegis flagged R4 UI gap blocking B1 Layer B — Mira packet dispatched to mount Sessions tab in V1 NavRail, P0; joint-review chapter fully closed at 32fbc72)*
